...
- **Three basic units exposed by MCP**
  ...
  | Unit | Description | Jira example |
  | --- | --- | --- |
  | **Tools** | Functions that AI can call | `createIssue`, `searchIssues`, `addComment` |
  | **Resources** | AI-readable data | Specific issue body, sprint information, board status |
  | **Prompts** | Predefined prompt templates | “Create a stand-up report”, “Sprint retrospective summary” |
...
![image](/assets/img/notion/TIL-JIRA-MCP-와-Harness-Engineering-특강/03-b804d67f9b.png)
  ...
  | **Permission Model** | Follow the same permissions the user has in Jira/Confluence |
...
This concept was formalized in Harness Engineering published by OpenAI in 2025.
...
- **Core Principle: Repositories are the only source of truth**<br>
  ...
  | Explaining workflow in Notion | Make your workflow an **executable script** |
...
- **Observability - Agents must also see logs**
  ...
  | --- | --- |
  | **Log** | Catching error patterns, specifying the point of occurrence |
  | **Metrics** | Find performance degradation areas |
...
- **Common Mistakes**
  ...
  | Mistake | Why it's a problem | What should I do |
  | --- | --- | --- |
  | Make AGENTS.md ambiguous | “Maintain code quality” → Not knowing what the agent will do | **Testable rules** such as "Functions must have less than 50 lines, JSDoc required for public functions" |
  | Absence of feedback loop | The agent assumes that he is correct and proceeds | Test/linter/type checker automation required |
  | Neglecting tacit knowledge | Agent mistakes increase as “you know it all” piles up | State the rules in your head as code/document |
  | Control all actions with script | If you kill autonomy too much, the agent's strength disappears | **Only provide direction** and delegate execution |
  | Harness made once and left behind | The model/project evolves, but only the harness stands still | Regularly inspected and updated |

### GitOps - A source of truth shared between humans and AI> GitOps
...
- **Four Principles of GitOps**
  ...
  | Principle | meaning |
  | --- | --- |
  | ① **Declarative** | “What should be” skills, not “how” |
...
- **GitOps Operation Flow**
  ...
  - Scenario example| action | Results |
...
- GitOps + Harness Engineering = AI-friendly infrastructure
  ...
  | **Rollback is unified** | Whether it is a human-made accident or an AI-made accident, it is equally recovered with `git revert` |
...
- Visibility equals productivity
  ...
  | --- | --- |
  | **Onboarding Speed** | New team members can self-study by following the code/document/issue/deployment flow |
  | **Incident Response Time** | Failure → Cause trace → Rollback shortened to minutes |
  | **AI Utilization** | Agents can take context, improving result quality |
  | **Decision-making speed** | No need to agree on “where are we now” in every meeting |

### The most important thing> **"Better environments, not better prompts, produce better results."**
