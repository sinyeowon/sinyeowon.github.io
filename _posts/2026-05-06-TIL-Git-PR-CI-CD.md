...
tags: ["Git", "CI/CD"]
description: "공부한 내용 Git Flow 브랜치 전략 브랜치 전략이 없다면, 내 작업이 동료 작업을 덮어쓰거나 / 배포 직전인데 미완성 코드가 섞이거나 / 롤백하고 싶은데 기준점이 없는 등의 사고가 발생할 수 있음 → 브랜치 전략 = 사고 방지 장치 Git Flow의 5가지 브랜치 | 브랜치 | 역"
...
```
    ...
    - 동작하는 순간 일어나는 일
      ...
      1. 한 단계라도 실패하면 PR에 x 표시

      1. 머지 버튼이 자동으로 비활성화 (Branch Protection 설정 시)
