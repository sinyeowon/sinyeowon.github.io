---
title: '[TIL] 스프링 숙련 TIL - 26/04/14'
date: 2026-04-14 16:08:22 +0900
last_modified_at: 2026-05-01 16:34:06 +0900
categories: ['Spring 단기 심화']
tags: ['Spring', 'TIL', '내일배움캠프']
description: 'JWT (Json Web Token): JSON 포맷을 이용하여 사용자에 대한 속성을 저장하는 Claim 기반의 Web Token일반적으로 쿠키 저장소를 사용해 JWT를 저장함서버가 1대인 경우Session1이 모든 Client의 로그인 정보를 소유함서버가 2대 이상인'
---
## 오늘 할 일
...
2. 서버가 2대 이상인 경우
    ...
    ![](https://velog.velcdn.com/images/sinyeowon/post/ad53f0ee-c3e6-4d94-87d0-9fa49179248f/image.png)
	...
	- 만약 Client1의 로그인 정보를 가지고 있지 않은 Server2나 Server3에 API 요청을 하게된다면
      - 해결 방법:
        ...
        2. 세션 저장소 생성해 모든 세션을 저장
          ![](https://velog.velcdn.com/images/sinyeowon/post/8b1bd7c5-f604-4a5f-b143-7914ed61e8f1/image.png)
...
- 로그인 정보를 Server에 저장하지 않고, Client에 로그인 정보를 JWT로 암호화하여 저장 -> JWT 통해 인증/인가
  ![](https://velog.velcdn.com/images/sinyeowon/post/46ab7bca-f4bc-403e-bbff-1ef7edaa212d/image.png)
- 모든 서버에서 동일한 Secret Key 소유
- Secret Key 통한 암호화 / 위조 검증 (복호화 시)
  ![](https://velog.velcdn.com/images/sinyeowon/post/5b09a1d7-bb86-4547-b419-441195d76396/image.png)
- 장점:
