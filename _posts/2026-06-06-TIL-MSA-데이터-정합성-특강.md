...
tags: ["Saga", "MSA"]
description: "Multiple Microservices에서는 예시로 주문(Order), 인보이스(Invoice), 결제(Payment), 배송(Shipping) 등이 각기 다른 서비스와 DB로 구성되어 협업한다."
...
- **원자적(원자성) 보장**: “DB 변경”과 “이벤트 발행”을 하나의 로컬 트랜잭션으로 묶기 위해 Outbox 테이블 기법을 사용 가능
    ...
    - Kafka 발행은
        ...
        **이슈 2: NoSQL DB 같이 데이터베이스가 트랜잭션을 지원하지 않는 경우**
